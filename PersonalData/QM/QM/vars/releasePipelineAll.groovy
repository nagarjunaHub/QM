...
def call(Map parameters) {
  ...
  def CUSTOMRELEASE = config.customRelease
  def release_info_file = config.release_all_info_file
  def RELEASE_TYPES = config.release_types.trim().tokenize(" ")
  def PACKAGE_RELEASE = config.package_release.trim().tokenize(" ")
  def RELEASE_TABLE = [
    "build_name": [
    ...
    "b2_qnx": [
      "release_dir": config.release_table.b2_qnx.release_dir,
      "version": B2_QNX_RELEASE_VERSION.trim(),
      "release_file": config.release_table.b2_qnx.release_file + ", release_note_${RELEASE_TYPE.toLowerCase()}*.log"
    ],
    "b2_ac": [
      "release_dir": config.release_table.b2_ac.release_dir,
      "version": B2_AC_RELEASE_VERSION.trim(),
      "release_file": config.release_table.b2_ac.release_file
    ],
    "b2_rse": [
      "release_dir": config.release_table.b2_rse.release_dir,
      "version": RSE_AOSP_RELEASE_VERSION.trim(),
      "release_file": config.release_table.b2_rse.release_file + ", release_note_${RELEASE_TYPE.toLowerCase()}*.log"
    ]
  ]
