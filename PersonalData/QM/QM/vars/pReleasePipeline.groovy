...
*/
  ...
  def artifactory_release = configMap.artifactory_release
  def artifactory_release_name = configMap.artifactory_release_name
  def MAX_DAY_TO_KEEP_RELEASE_BUILD=configMap.max_day_to_keep_release_build
  def MAX_DAY_TO_KEEP_TEST_BUILD=configMap.max_day_to_keep_test_build
  def SEND_FROM = configMap.general.default_email_sender
  def CUSTOMRELEASE = configMap.general.customRelease
  def release_info_file = configMap.release_info_file
  def RELEASE_TYPES = configMap.release_types.trim().tokenize(" ")
  def PACKAGE_RELEASE = (RELEASE_TYPE.contains("nightly")) ? configMap.package_release_nightly.trim().tokenize(" ") : configMap.package_release.trim().tokenize(" ")
  def PACKAGE_CLEANUP = configMap.package_cleanup.trim().tokenize(" ")
  def RELEASE_TABLE = [
    "build_name": [
      ...
      "snapshot_job": configMap.release_table.build_name.snapshot_job
    ],
    "qnx": [
      "release_dir": configMap.release_table.qnx.release_dir,
      "version": QNX_RELEASE_VERSION.trim(),
      "release_file": configMap.release_table.qnx.release_file,
      "snapshot_job": configMap.release_table.qnx.snapshot_job
    ],
    "aosp": [
      "release_dir": configMap.release_table.aosp.release_dir,
      "version": AOSP_RELEASE_VERSION.trim(),
      "release_file": configMap.release_table.aosp.release_file,
      "snapshot_job": configMap.release_table.aosp.snapshot_job
    ],
    "ac": [
      "release_dir": configMap.release_table.ac.release_dir,
      "version": AC_RELEASE_VERSION.trim(),
      "release_file": configMap.release_table.ac.release_file,
      "snapshot_job": configMap.release_table.ac.snapshot_job
    ],
    "app": [
      "release_dir": configMap.release_table.app.release_dir,
      "version": AOSP_RELEASE_VERSION.trim(),
      "release_file": configMap.release_table.app.release_file,
      "snapshot_job": configMap.release_table.app.snapshot_job
    ],
    "get_flash": [
      "release_dir": configMap.release_table.get_flash.release_dir,
      "version": "",
      "release_file": configMap.release_table.get_flash.release_file,
      "snapshot_job": configMap.release_table.get_flash.snapshot_job
    ]
  ]

  def iip_release_dir = configMap.iip_release_dir
          ...
          stage("Generating: ${it}") {
            ...
            if (it.contains("qnx")) {
                def get_baseline_iip_version = sh(returnStdout:true, script:"""#!/bin/bash -ex
                  ...
                  source ${bashLib} && _git_clone_lite ${configMap.general.baselineBranch} ssh://${configMap.general.gerritServer}:29418/${configMap.general.manifestRepo} \${iip_check_dir}
